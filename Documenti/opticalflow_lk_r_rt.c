...
LIBMWVISIONRT_API void MWVIP_OpticalFlow_LK_R( const real32_T  *inImgA,  /* current */
                                       ...
                                       int_T   inCols)
{
    ...
    {
        real32_T tmpGradR = gradRR[j];  // Ix
        real32_T tmpGradC = gradCC[j]; // Iy
        real32_T tmpGradT = inImgB[j] - inImgA[j]; /* GradT */   // It

        gradRR[j] = tmpGradR*tmpGradR;  // Ix^2
        gradCC[j] = tmpGradC*tmpGradC;  // Iy^2
        gradRC[j] = tmpGradR*tmpGradC;  // Ix*Iy

        gradRT[j] = tmpGradR*tmpGradT; // Ix*It
        gradCT[j] = tmpGradC*tmpGradT; // Iy*It
    }
