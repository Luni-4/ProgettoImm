...
%     end
    ...
    uStimato = zeros(size(regioni,1),size(regioni,2),size(ccp1,1));
    vStimato = uStimato;
    
    % Calcolo u e v rispetto ogni centro possibile
    for i=1:size(ccp1,1)
        uStimato(:,:,i) = reshape(regressore*(ccp1(i,:))',sz);
        vStimato(:,:,i) = reshape(regressore*(ccp2(i,:))',sz);
    end
    
    % Ripeto valori di u e v trovati dal flusso ottico per numero centri
    % possibili
    uRipetuta = repmat(u,[1,1,size(ccp1,1)]);
    vRipetuta = repmat(v,[1,1,size(ccp1,1)]);
    
    % Trovo a quali centri sono più vicini tutti i punti
    uDist = (uRipetuta - uStimato).^2;
    vDist = (vRipetuta - vStimato).^2;
